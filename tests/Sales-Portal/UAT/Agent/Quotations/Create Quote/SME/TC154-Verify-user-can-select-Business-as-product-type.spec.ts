...
test.describe.serial("@TC154 @Regression Verify user can select 'Business' as product type", () => {
  let page: Page;
  let pm: PageManager2;
  let browser: Browser;
  ...
  test("Verify Selecting SME of Business", async () => {
    await test.step("Click on SME", async () => {
      await expect(pm.inQuotationsPage().smeOption).toBeVisible({ timeout: 10000 });
