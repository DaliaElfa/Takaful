...
test.describe
  .serial("@TC156 @Regression  Verify correct plan types are displayed for Business product", () => {
  let page: Page;
  let pm: PageManager2;
  let browser: Browser;
  ...
  >("test-data/SmeData.json");
  const customer_user = customerinfo[0];
  ...
  test("Verify Selecting SME", async () => {
    await test.step("Click on Business 'SME' option", async () => {
      await expect(pm.inQuotationsPage().smeOption).toBeVisible({
        timeout: 10000,
      });

      await pm.inQuotationsPage().clickOnSME();
    ...
    await test.step("Select Motor class of business & fill customer info", async () => {
      await pm.inQuotationsPage().fillTheCRfield(customer_user.CR);
      await pm
        .inQuotationsPage()
        .fillSMECustomerInformation(
