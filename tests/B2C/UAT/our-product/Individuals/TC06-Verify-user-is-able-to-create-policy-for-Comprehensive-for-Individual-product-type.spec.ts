...
test.describe
  .serial("@TC06 @Regression  Verify user is able to create policy for 'Comprehensive' for Individual product type", () => {
  let page: Page;
  let pm: PageManager;
  let browser: Browser;
  ...
  test.beforeAll(async () => {
    await test.step("Launch the URL", async () => {
      const result = await openBrowser("edge", JsonReader.getEnv("uat1").baseUrl);
  ...
  test("Fill OTP Code", async () => {
    await test.step("Fill first OTP container", async () => {
      ...
      await pm.InVerificationCodePage().waitForLoad(/motor-individual-quotation-stepper/);
    });
  });

  test("Doing Actions in Premium Calculations Page", async () => {
    await test.step("Click on comprehansive insurance container", async () => {
      await pm.InPremiumCalculationPage().ClickOnComprehensiveInsurance();
      await pm.InPremiumCalculationPage().Add_AddOns();
      await pm.InPremiumCalculationPage().clickOnAddNewDriver();
      await pm.InPremiumCalculationPage().fillDriverInfo();
    });
  });
